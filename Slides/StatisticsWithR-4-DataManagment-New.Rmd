---
title: "Statistical Analysis with R: Data Management and Automation"
author: "Alex Sanchez"
format: revealjs
editor: visual
---
...
*Based on this presentation: [Data Managment, UCLA](https://stats.idre.ucla.edu/stat/data/rdm/data_management_seminar.html#153).
...
diab_filt <- dplyr::filter(diab, tabac == "No fumador", edat >= 50)
head(diab_filt, n = 4)
```
...
```{r}
diab_small <- dplyr::select(diab, mort, edat, tabac, sbp)
head(diab_small, n = 4)
```
...
diab_sort <- dplyr::arrange(diab, desc(mort), edat) 
head(diab_sort, n = 4)
```
